...
const SingleListItem = ({ course }) => {
  return (
    <motion.div
      ...
      whileHover={{ y: -9 }}
      className="rounded-lg overflow-hidden shadow-lg bg-slate-900 transition-all duration-300 group p-5 sm:p-7 flex flex-col md:flex-row items-start md:items-center gap-6 hover:shadow-2xl hover:shadow-orange-500"
    >
      ...
      <div className="w-full flex-1 mt-0">
        ...
        <div className="flex flex-wrap gap-2 my-5">
          {course.technologies.slice(0, 4).map((tech) => (
            <span
              key={tech.name}
              className="flex items-center gap-1 px-2 py-1 bg-orange-400/40 rounded-md text-xs"
            >
